...
PoissonEquation::PoissonEquation() : fe(1), dof_handler(triangulation) {}

void PoissonEquation::make_grid()
{
...
void PoissonEquation::setup_system()
{
...
void PoissonEquation::assemble_system()
{
    ...
    for (const auto &cell : dof_handler.active_cell_iterators())
    {
        ...
        for (const unsigned int q_index : fe_values.quadrature_point_indices())
        {
            ...
            for (const unsigned int i : fe_values.dof_indices())
                cell_rhs(i) += (fe_values.shape_value(i, q_index) * fe_values.JxW(q_index));
        }
...
void PoissonEquation::output_results() const
{
    DataOut<2> data_out;
...
int main()
{
    ...
    PoissonEquation laplace_problem;
    laplace_problem.run();
    return 0;
}
